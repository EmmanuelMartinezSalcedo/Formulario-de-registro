...
from wtforms import StringField, PasswordField, SubmitField, DateField, SelectField, FileField, IntegerField
...
class RegisterForm(FlaskForm):
    ...
    birthday = DateField('birthday', format='%Y-%m-%d', validators=[InputRequired()],render_kw={"placeholder": "Fecha de nacimiento"})
...
class ProductForm(FlaskForm):
    name = StringField(validators=[
                           InputRequired(), Length(min=4, max=20)], render_kw={"placeholder": "Nombre"})
    price = IntegerField(validators=[
                           InputRequired()], render_kw={"placeholder": "Precio"})
...
def product():
    form = ProductForm()
    if form.validate_on_submit():
        new_product = images(name=form.name.data, img=form.image.data, price=form.price.data)
